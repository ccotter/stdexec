...
struct async_scope_request_stop : rl::test_suite<async_scope_request_stop, 1>
{
    static size_t const dynamic_thread_count = 1;
    ...
    void thread(unsigned)
    {
        ...
        exec::async_scope scope;
        ex::sender auto begin = ex::schedule(sch);
        ex::sender auto ftr = scope.spawn_future(begin | stdexec::then([&]() { return 1; }));
        ex::sender auto ftr_then = scope.spawn_future(ex::let_value(begin, [&] {
            return std::move(ftr) | stdexec::then([&](int v) { return v+1; });
        }));
        scope.request_stop();
        stdexec::sync_wait(stdexec::when_all(scope.on_empty(), std::move(ftr_then)));
    }
...
int main()
{
    ...
    rl::simulate<async_scope_request_stop>(p);
    return 0;
}
